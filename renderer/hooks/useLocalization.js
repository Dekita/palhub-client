...
export const LocalizationProvider = ({ children }) => {
    ...
    const innerT = React.useCallback(
        (pointkey) => {
    ...
    const innerM = React.useCallback(
        (bundle_point = '', replacers = {}) => {
    ...
    const t = React.useCallback(
        (keystring, replacers = {}, expectedArraySize = null) => {
            let bundle_point = innerT(keystring);
            if (Array.isArray(bundle_point)) {
                // handle array of strings
                try {
                    return bundle_point.map(innerT).map((e) => innerM(e, replacers));
                } catch (error) {
                    console.error(e);
                }
                return [];
    ...
    const tA = React.useCallback(
        (keystring, replacersOrSize = {}, expectedSize = 1) => {
    ...
    const onUpdateLanguage = React.useCallback(
        async (locale, namespace = undefined) => {
